...
let add_to_previous prev_names name = 
  if is_previously_named prev_names name then
    raise (Failure ("function " ^ name ^ " is previously defined"))
  else
    name :: prev_names

(* names all functions *)
let name_all (typ_decls, sexpr) =
  let rec name_all' (ty, sx) prev_names is_named opt_name = match sx with
  ...
  | SLet (bound_vars, body) ->
      let (bound_vars', prev_names') = List.fold_left
        (fun (bound_vars, prev_names) ((name, ty), sexpr) ->
          let (sexpr', prev_names') = name_all' sexpr prev_names true name
  ...
  | SFunction (params, body) ->
    let (body', prev_names') = name_all' body prev_names false ""
      in if is_named
        then ((ty, SFunction (params, body')), add_to_previous prev_names' opt_name)
        else
  ...
  | SStructRef (struct_name, field) ->
    (* let prev_names' = add_to_previous prev_names struct_name in *)
      ((ty, SStructRef (struct_name, field)), prev_names)
  | SCall (fsexpr, sexprs) ->
