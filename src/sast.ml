...
let string_of_sprogram_lifted (typ_decls, functions, expr) =
  "functions:\n\n" ^ (String.concat "\n" (List.map (fun (bind, sexpr) -> string_of_bind bind ^ " = " ^ string_of_sexpr sexpr) functions))
  ^ "\n\neverything else:\n\n" ^ string_of_sprogram (typ_decls, expr)
